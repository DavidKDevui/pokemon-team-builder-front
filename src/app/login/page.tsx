...
export default function LoginPage() {
  ...
  const isSubmitDisabled = isSubmitting || !emailRegex.test(email) ||  !passwordRegex.test(password) ;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (response instanceof Error) {
        ...
        if (statusCode === 400 || statusCode === 500) {
          ...
          toastService.error("Identifiants incorrects");
        } else {
            toastService.error("Erreur interne du serveur");
        }
      ...
      router.push('/team');
      toastService.success("Connexion réussie ! Content de vous revoir !");
      setIsSubmitting(false);
    }
  };
        ...
        {!isAuthenticated  && isAuthenticated !== undefined && (
            ...
            <button
                ...
                <p className="mt-2 text-white/70 font-press-start-2p text-sm">
                    Accédez à votre équipe Pokémon
                </p>
                </div>
